...
for info in os.listdir(input_path):
    ...
    for k in range(start_item, end_item):
        ...
        theta_value_i = sum(theta.T.tolist()[0])
        # theta_value_i = sum(list(map(abs, theta.T.tolist()[0])))
        theta_value.append(theta_value_i)

        # if theta_value_i >150:
        #     print(theta)
        #     print(theta_value_i)
        #     exit()

        # print(len(theta_value), theta_value)
    p = np.eye(25) * (10 ** 8)  # 每个文件重置一次
...
# print(theta_value)
print('均值为: ', np.mean(theta_value))
print('最大值: {}, 最小值: {}'.format(max(theta_value), min(theta_value)))
# print(p.shape, p)

plt.plot(theta_value, color='red', linewidth=2, linestyle='--', label='linear line')
plt.title('theta')
plt.show()
"""
8 月 不重置p theta
